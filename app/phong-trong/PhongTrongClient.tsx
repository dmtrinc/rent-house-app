...
function getAvailInfo(dateStr?: string | null) {
  ...
  if (diff < 30) return { label: "Trống " + formatDateVN(av), type: "soon" };
  return { label: "Trống " + formatDateVN(av), type: "late" };
}
...
function buildHaystack(listing: Listing): string {
  ...
  return removeDiacritics([
    ...
    listing.availableDate ? formatDateVN(listing.availableDate) : "trống sẵn",
    ...(listing.highlights ?? []),
    ...(listing.amenities  ?? []),
    listing.description ?? "",
    listing.category    ?? "",
...
export interface PhongTrongClientProps {
  /** Tin active render sẵn từ server (page.tsx). */
  initialItems: Listing[];
  /** Tiêu đề/footer đọc từ system_config trên server. */
  initialTitle: string;
  initialFooter: string;
}

export default function PhongTrongClient({ initialItems, initialTitle, initialFooter }: PhongTrongClientProps) {
  // Ưu tiên cache client (quay lại trang) → dữ liệu server → rỗng (skeleton)
  const [items,   setItems]   = useState<Listing[]>(() => phongTrongCache?.items ?? initialItems);
  const [loading, setLoading] = useState(() => !phongTrongCache && initialItems.length === 0);
  ...
  const [pageTitle,  setPageTitle]  = useState(() => phongTrongCache?.title  ?? initialTitle  ?? DEFAULT_PHONGTRONG_TITLE);
  const [footerText, setFooterText] = useState(() => phongTrongCache?.footer ?? initialFooter ?? DEFAULT_PHONGTRONG_FOOTER);
  const [editTitle,  setEditTitle]  = useState(() => phongTrongCache?.title  ?? initialTitle  ?? DEFAULT_PHONGTRONG_TITLE);
  const [editFooter, setEditFooter] = useState(() => phongTrongCache?.footer ?? initialFooter ?? DEFAULT_PHONGTRONG_FOOTER);
    ...
    try {
      ...
      if (res.ok) {
        setItems(prev => prev.map(i =>
          i._id === room._id ? { ...i, price, availableDate: availableDate || null, status, costs } : i
        ));
        setEditDraft(p => p ? { ...p, saving: false, saveMsg: "✅ Đã lưu" } : null);
        setTimeout(() => setEditDraft(null), 1200);
      } else {
        setItems(prev => prev.map(i => i._id === room._id ? { ...i, costs } : i));
  ...
  const skRow = (
    <tr style={{ borderBottom: "1px solid #f0f0f0" }}>
      <td style={{ padding: "8px", width: 24 }}>
        <div style={{ height: 12, width: 16, borderRadius: 4, background: "linear-gradient(90deg,#f0f0f0 25%,#e8e8e8 50%,#f0f0f0 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.4s infinite" }} />
      </td>
      <td style={{ padding: "8px", minWidth: 160 }}>
        <div style={{ height: 12, borderRadius: 4, marginBottom: 4, background: "linear-gradient(90deg,#f0f0f0 25%,#e8e8e8 50%,#f0f0f0 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.4s infinite" }} />
        <div style={{ height: 10, width: "60%", borderRadius: 4, background: "linear-gradient(90deg,#f0f0f0 25%,#e8e8e8 50%,#f0f0f0 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.4s infinite" }} />
      </td>
      {[80, 60, 70, 50, 50, 50, 70].map((w, j) => (
        <td key={j} style={{ padding: "8px" }}>
          <div style={{ height: 12, width: w, borderRadius: 4, background: "linear-gradient(90deg,#f0f0f0 25%,#e8e8e8 50%,#f0f0f0 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.4s infinite" }} />
        </td>
      ))}
    </tr>
  );
    ...
    <div style={{ minHeight: "100vh", background: "#f8f8f8" }}>
      ...
      <header style={{ background: GREEN, borderBottom: "1px solid #004d26", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 6px rgba(0,0,0,0.15)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {canEdit && (
              <>
                <button onClick={() => setEditMode(v => !v)} className={editMode ? "header-btn header-btn--edit-on" : "header-btn header-btn--edit-off"}>
                  {editMode ? "✏️ Đang sửa" : "✏️ Chỉnh sửa"}
                </button>
                <button onClick={() => setAdminOpen(v => !v)} className="header-btn header-btn--yellow">
                  ⚙️ {isAdmin ? "Admin" : "Mod"}
                </button>
              </>
            )}
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "14px 14px 0" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 12, overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.07)", marginBottom: 24 }}>
          <div style={{ padding: "8px 12px 6px", borderBottom: "1px solid #e8f5e9", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 4 }}>
            <h1 style={{ fontSize: 13, fontWeight: 700, color: GREEN, margin: 0 }}>{pageTitle}</h1>
            <div style={{ fontSize: 10, color: "#999", fontStyle: "italic", whiteSpace: "nowrap" }} suppressHydrationWarning>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
              ...
              <tbody>
                {loading
                  ? Array.from({ length: 8 }).map((_, i) => <React.Fragment key={i}>{skRow}</React.Fragment>)
                  : sorted.length === 0
                  ? (
                    <tr>
                      <td colSpan={9} style={{ padding: "40px 20px", textAlign: "center", color: "#aaa", fontSize: 13 }}>
                  ...
                  : sorted.map((room, idx) => {
                    ...
                    return (
                      <tr key={room._id}
                        style={{ borderBottom: "1px solid #f0f0f0", cursor: "pointer", opacity: isHidden ? 0.6 : 1, background: isHidden ? "#fff8f8" : undefined }}
                        onMouseEnter={interactiveReady ? (e => (e.currentTarget.style.background = editMode && canEdit ? "#fffde7" : "#f0faf5")) : undefined}
                        onMouseLeave={interactiveReady ? (e => (e.currentTarget.style.background = isHidden ? "#fff8f8" : "#fff")) : undefined}
                        onClick={() => { if (canEdit && editMode) openEditDraft(room); else setSelected(room); }}>
          ...
          <div style={{ background: "#fff", borderRadius: 16, maxWidth: 460, width: "100%", maxHeight: "88vh", overflowY: "auto", boxShadow: "0 8px 32px rgba(0,0,0,0.18)" }}
            ...
            <div style={{ background: GREEN, borderRadius: "16px 16px 0 0", padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                ...
                {selected.contactPhone && <div style={{ color: "rgba(255,255,255,0.85)", fontSize: 12, marginTop: 2 }}>📞 {selected.contactPhone}</div>}
              </div>
            ...
            <div style={{ padding: "16px 18px" }}>
              ...
              {([
                { label: "Giá thuê",   value: <span style={{ color: GREEN, fontSize: 15, fontWeight: 700 }}>{formatPrice(selected.price)} đ/tháng</span> },
            ...
            <div style={{ padding: "16px 18px", display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                ...
                <input type="number" value={editDraft.price} min={0} step={100000}
                  ...
                  style={inputStyle} />
                <div style={{ fontSize: 10, color: "#aaa", marginTop: 2 }}>= {(editDraft.price / 1_000_000).toFixed(2)}tr</div>
              </div>
              <div>
                ...
                <input type="date" value={editDraft.availableDate}
                  ...
                  style={inputStyle} />
                <div style={{ fontSize: 10, color: "#aaa", marginTop: 2 }}>Để trống = trống sẵn ngay</div>
              </div>
              <div>
                  ...
                  {(["active", "hide"] as const).map(s => (
                    <button key={s} onClick={() => setEditDraft(p => p ? { ...p, status: s } : null)}
                      style={{ flex: 1, padding: "8px", borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: "pointer",
                        background: editDraft.status === s ? (s === "active" ? GREEN : "#c00") : (s === "active" ? "#e8f5e9" : "#fee"),
                  ...
                  {([
                    ...
                    <div key={key}>
                      ...
                      <input type={type} value={editDraft.costs[key] as string} placeholder={placeholder}
                        onChange={e => setEditDraft(p => p ? { ...p, costs: { ...p.costs, [key]: type === "number" ? Number(e.target.value) : e.target.value } } : null)}
                        style={inputStyle} />
                    </div>
                  ))}
                </div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={saveEditDraft} disabled={editDraft.saving}
                  style={{ flex: 2, padding: "10px", borderRadius: 10, border: "none", background: editDraft.saving ? "#aaa" : GREEN, color: "#fff", fontSize: 13, fontWeight: 700, cursor: editDraft.saving ? "not-allowed" : "pointer" }}>
              ...
              <div style={{ fontSize: 10, color: "#bbb", textAlign: "center" }}>
                * Giá, ngày trống, trạng thái lưu qua PATCH /api/listings/id. Chi phí lưu local.
              </div>
            </div>
          </div>
        </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        * { box-sizing: border-box; }
        .header-btn {
          display: inline-block; padding: 5px 12px; border-radius: 22px;
          font-size: 12px; font-weight: 600; cursor: pointer; text-decoration: none;
          border: 1px solid transparent;
          transition: background 0.18s, color 0.18s, box-shadow 0.18s, transform 0.12s;
        }
        .header-btn:hover { transform: translateY(-1px); box-shadow: 0 3px 10px rgba(0,0,0,0.18); }
        .header-btn--yellow { border-color: rgba(255,255,255,0.3); color: #FFD966; background: rgba(255,255,255,0.1); }
        .header-btn--yellow:hover { background: rgba(255,215,0,0.22); color: #ffe680; }
        .header-btn--ghost { border-color: rgba(255,255,255,0.3); color: #fff; background: rgba(255,255,255,0.1); }
        .header-btn--ghost:hover { background: rgba(255,255,255,0.22); }
        .header-btn--edit-off { border-color: rgba(255,255,255,0.3); color: rgba(255,255,255,0.8); background: rgba(255,255,255,0.08); }
        .header-btn--edit-off:hover { background: rgba(255,255,255,0.18); color: #fff; }
        .header-btn--edit-on { border-color: #ffc107; color: #333; background: #ffc107; animation: pulse-yellow 1.8s ease-in-out infinite; }
        .dang-tin-btn {
          display: inline-block; padding: 5px 13px; border-radius: 22px;
          border: 1px solid #d4a800; color: #006633; background: #FFD966;
          font-size: 12px; font-weight: 700; text-decoration: none;
          transition: background 0.2s, transform 0.15s, box-shadow 0.2s;
        }
        .dang-tin-btn:hover { background: #ffd27a; box-shadow: 0 2px 8px rgba(180,120,0,0.22); transform: translateY(-1px); animation: wiggle 0.5s ease; }
      `}} />
    </div>
  );
}
