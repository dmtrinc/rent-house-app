...
function getAvailabilityInfo(availableDate: string | null | undefined) {
  ...
  if (diff < 30) return { label: `Trống từ ${formatDateVN(avail)}`, type: "soon", color: "#b08500", bg: "#fff8e1" };
  return { label: `Trống từ ${formatDateVN(avail)}`, type: "late", color: "#888", bg: "#f5f5f5" };
}
      ...
      <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
        ...
        <div style={{ padding: 12 }}>
          ...
          <div style={{ fontWeight: 800, fontSize: 16, color: "#006633" }}>
            {formatPrice(item.price)} đ<span style={{ fontSize: 12, fontWeight: 400, color: "#888" }}>/tháng</span>
          </div>
        </div>
      </div>
    </Link>
...
export interface ListingDetailClientProps {
  id: string;
  /** Dữ liệu render sẵn từ server (page.tsx). null → hiện skeleton rồi fetch. */
  initialData: ListingDoc | null;
  initialSimilar: ListingDoc[];
}

export default function ListingDetailClient({ id, initialData, initialSimilar }: ListingDetailClientProps) {
  // Ưu tiên cache client (đã xem rồi quay lại) → dữ liệu server → null (skeleton)
  const [data, setData] = useState<any>(() => detailCache.get(id)?.data ?? initialData);
  const [similar, setSimilar] = useState<any[]>(() => detailCache.get(id)?.similar ?? initialSimilar);
  ...
  // Ảnh hero có trong HTML server có thể tải xong TRƯỚC khi React hydrate → onLoad
  // không bao giờ chạy. Kiểm tra img.complete sau mount để không kẹt opacity 0.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/listings/${id}`, { cache: "no-store" })
      ...
      .catch(() => { if (!detailCache.get(id) && !initialData) alert("Không thể tải thông tin!"); });
  }, [id]);
        ...
        <div style={{ background: "#fff", borderRadius: 14, padding: "16px 20px", marginBottom: 12, boxShadow: "0 1px 6px rgba(0,0,0,0.07)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10, flexWrap: "wrap", gap: 8 }}>
            <div style={{ fontSize: "clamp(22px,4vw,28px)", fontWeight: 900, color: GREEN, lineHeight: 1 }}>
              {formatPrice(data.price)} <span style={{ fontSize: 14, fontWeight: 500, color: "#777" }}>đ/tháng</span>
            </div>
