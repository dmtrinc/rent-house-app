      ...
      <div style={{ background: "#fff", borderRadius: 16, maxWidth: 440, width: "100%",
        ...
        <div style={{ padding: "18px 18px 24px" }}>
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#666", marginBottom: 4 }}>💰 Giá thuê (đ/tháng)</label>
            <input type="number" value={price} onChange={e => setPrice(Number(e.target.value))} style={inputSm} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
            {[
              { label: "⚡ Điện (đ/kWh)", val: elec, set: setElec },
              { label: "💧 Nước (đ/ng)", val: water, set: setWater },
              { label: "🧹 Dịch vụ (đ/P)", val: sv, set: setSv },
              { label: "🚲 Đậu xe (đ/xe)", val: bike, set: setBike },
            ].map(({ label, val, set }) => (
              <div key={label}>
                <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#666", marginBottom: 4 }}>{label}</label>
                <input type="number" value={val} onChange={e => set(Number(e.target.value))} style={inputSm} />
              </div>
            ))}
          </div>
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#666", marginBottom: 4 }}>📅 Ngày trống</label>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={onClose}
              style={{ flex: 1, padding: "10px 0", border: "1px solid #ddd", borderRadius: 10,
                background: "#f5f5f5", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Huỷ</button>
      ...
      <div style={{ background: "#fff", borderRadius: 16, maxWidth: 460, width: "100%",
        ...
        <div style={{ padding: "16px 18px" }}>
          ...
          {[
            ...
            { label: "💧 Nước",       value: `${fmtMoney(costs.water)} đ/ng` },
            { label: "🧹 Dịch vụ",    value: `${fmtMoney(costs.sv)} đ/P` },
            ...
            {canEdit && (
              <button onClick={() => { onClose(); onQuickEdit(item); }}
                style={{ flex: 1, padding: "10px 0", border: `1.5px solid ${GREEN}`, borderRadius: 10,
                  background: "#e8f5e9", color: GREEN, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                ✏️ Sửa nhanh
              </button>
            )}
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, ...extraStyle }}>
      ...
      <button onClick={() => setEditing(true)}
        style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)",
          borderRadius: 7, color: "#fff", fontSize: 13, cursor: "pointer", padding: "2px 7px", flexShrink: 0 }}>🖊️</button>
    </div>
  );
}

export default function UserListingPage({ params }: { params: Promise<{ user: string }> }) {
  const { user: rawUsername } = React.use(params);
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res  = await fetch(`/api/user/${encodeURIComponent(username)}/listing`);
  ...
  const thBase: React.CSSProperties = {
    ...
    fontSize: 11, fontWeight: 600, whiteSpace: "nowrap", background: GREEN, position: "sticky", top: 0,
  };
  ...
  if (error) return (
    <div style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 }}>
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#f2f4f3", fontFamily: "'Be Vietnam Pro','Segoe UI',sans-serif" }}>
            ...
            {(currentUser as any)?.username ? (
              <button onClick={() => router.push("/user")}
                style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(255,255,255,0.15)",
                  border: "1.5px solid rgba(255,255,255,0.3)", borderRadius: 20, padding: "4px 4px 4px 10px",
                ...
                <div style={{ lineHeight: 1.2 }}>
                  ...
                  <div style={{ fontSize: 10, fontWeight: 700,
                    color: (currentUser as any).role === "admin" ? "#FF8C00" : (currentUser as any).role === "mod" ? "#ce93d8" : "rgba(255,255,255,0.65)" }}>
                ...
                <div style={{ width: 30, height: 30, borderRadius: "50%",
                  ...
                  border: "2px solid rgba(255,255,255,0.5)", flexShrink: 0, marginLeft: 4 }}>
                  {(currentUser as any).username?.[0]?.toUpperCase()}
                </div>
              </button>
            ) : (
              <button onClick={() => router.push("/login")}
                style={{ padding: "6px 14px", borderRadius: 20, border: "1px solid rgba(255,255,255,0.3)",
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 0 50px" }}>
        ...
        {isEditMode && (
          <div style={{ display: "flex", gap: 8, padding: "10px 16px" }}>
            {[
              ...
              <div key={label} style={{ flex: 1, background: bg, borderRadius: 8, padding: "6px 8px", textAlign: "center", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
        ...
        <div style={{ margin: isEditMode ? "0 16px" : "14px 16px", borderRadius: 12, overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.09)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
            ...
            {isEditMode && (
              <button onClick={() => { const v = window.prompt("Sửa tiêu đề:", headerText); if (v !== null) saveText("headerText", v); }}
                    ...
                    {list.map((item, idx) => {
                      ...
                      return (
                        <tr key={item._id}
                          ...
                          onMouseEnter={e => e.currentTarget.style.background = isHidden && isEditMode ? "#fff0f0" : "#f0faf5"}
                          onMouseLeave={e => e.currentTarget.style.background = isHidden && isEditMode ? "#fff8f8" : "#fff"}
                          ...
                          {isEditMode && (
                            <td style={td} onClick={e => e.stopPropagation()}>
                              <button onClick={() => setQuickEditItem(item)}
                                style={{ padding: "3px 9px", borderRadius: 7, border: `1px solid ${GREEN}`,
                                  background: "#e8f5e9", color: GREEN, fontSize: 11, fontWeight: 600, cursor: "pointer" }}
          ...
          {/* Footer bar */}
          <div style={{ display: "flex", alignItems: "center", padding: "9px 14px", gap: 8, background: GREEN, borderRadius: "0 0 10px 10px" }}>
            {isEditMode
      ...
      {quickEditItem && (
        <QuickEditModal item={quickEditItem} onClose={() => setQuickEditItem(null)} onSaved={handleQuickSaved} />
      )}
