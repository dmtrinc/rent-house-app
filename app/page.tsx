...
export default async function Home() {
  ...
  return (
    <main style={{ backgroundColor: "#000", minHeight: "100vh", padding: "40px 20px", color: "#fff" }}>
        ...
        {listings.map((item: any) => (
          <div key={item._id} style={{ backgroundColor: "#111", borderRadius: "15px", overflow: "hidden", border: "1px solid #222", position: "relative" }}>
            ...
            <div style={{ padding: "15px" }}>
              <h2 style={{ fontSize: "18px", marginBottom: "10px" }}>{item.title || item.name || "Không tiêu đề"}</h2>
              <p style={{ color: "#ff4d4d", fontSize: "20px", fontWeight: "bold" }}>
                {item.price ? Number(item.price).toLocaleString("vi-VN") : "0"} đ
              </p>
              <p style={{ color: "#aaa", fontSize: "14px", marginTop: "10px" }}>📍 {item.address || "Chưa có địa chỉ"}</p>
            </div>
          </div>
        ))}
      </div>
