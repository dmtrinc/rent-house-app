...
export interface HomeClientProps {
  /** Tin active đã sort sẵn từ server (page.tsx) — có ngay trong HTML cho bot. */
  initialItems: ListingDoc[];
...
export default function HomeClient({ initialItems, initialConfig, intro }: HomeClientProps) {
  ...
  const [visibleItems, setVisibleItems] = useState<any[]>(() =>
    homeCache ? homeCache.items.slice(0, homeCache.page * PAGE_SIZE) : initialItems.slice(0, PAGE_SIZE));
  const [page, setPage] = useState(() => homeCache?.page ?? 1);
  const [hasMore, setHasMore] = useState(() =>
    homeCache ? homeCache.items.length > homeCache.page * PAGE_SIZE : initialItems.length > PAGE_SIZE);
  const [user, setUser] = useState<any>(null);
  const [myDeviceId, setMyDeviceId] = useState("");
  const [systemConfig, setSystemConfig] = useState<any>(() => homeCache?.config ?? initialConfig ?? { globalPostEnabled: true });
  const [loading, setLoading] = useState(() => !homeCache && initialItems.length === 0);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      } catch { if (!homeCache && !initialItems.length) setAllItems([]); }
      finally {
        setLoading(false);
